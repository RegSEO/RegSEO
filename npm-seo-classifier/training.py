...
class Loader:
    ...
    def load_data(self):
        # black_features, black_labels = self.black_loader.load_data()
        # black_features_train, black_features_test, black_labels_train, black_labels_test = train_test_split(
        #     black_features, black_labels, test_size=0.7, random_state=0)

        white_features, white_labels = self.white_loader.load_data()
        white_features_train, white_features_test, white_labels_train, white_labels_test = train_test_split(
            white_features, white_labels, test_size=0.7, random_state=0)
...
class ModelTrainer:
    ...
    def train(self, modelfile='./model/RFC_v3.joblib', datafile='./model/train_data_v3.joblib'):
...
class ModelTester:
    ...
    def test(self, datafile='./model/test_data_v3.joblib'):
        # self.load_test_data()
        self.predict()
...
if __name__ == "__main__":
    ...
    print("start loading")
    word2vec = KeyedVectors.load_word2vec_format('/data/black_seo_research/npm-seo-classifier/model/wiki.en.vec') # 奇安信服务器上的版本
    # word2vec = KeyedVectors.load_word2vec_format('./model/wiki-news.vec')
    print("finished")

    classifier = RandomForestClassifier(n_estimators=100)

    black_loader = DataLoader(label_file_path='/data/black_seo_research/npm-seo-classifier/data/test_lable_black_v3.xlsx', model=word2vec, flag='black')
    white_loader = DataLoader(label_file_path='/data/black_seo_research/npm-seo-classifier/data/test_lable_white_v3.xlsx', model=word2vec, flag='white')
    # black_loader = DataLoader(label_file_path='/data/black_seo_research/npm-seo-classifier/data/black_v3.xlsx', model=word2vec, flag='black')
    # white_loader = DataLoader(label_file_path='/data/black_seo_research/npm-seo-classifier/data/white_v3.xlsx', model=word2vec, flag='white')
